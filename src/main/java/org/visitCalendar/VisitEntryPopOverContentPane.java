...
public class VisitEntryPopOverContentPane extends PopOverContentPane {

    public VisitEntryPopOverContentPane(VisitEntry entry, List<Calendar> allCalendars){
        //EntryHeaderView header = new EntryHeaderView(entry,allCalendars);
        requireNonNull(entry);
        EntryHeaderView header = new EntryHeaderView(entry, allCalendars);
        header.getChildren().get(2).setVisible(false);
        VisitEntryDetailsView details = new VisitEntryDetailsView(entry);
